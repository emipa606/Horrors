...
{
    private readonly List<NoiseLayer> layers;
    ...
    {
        var l = new NoiseLayer { amp = amp, freq = freq, perm = new int[512] };
        for (var i = 0; i < 256; i++)
        {
    ...
    public double layeredNoise(double x, double y)
    {
        ...
        {
            maxSum += noiseLayer.amp;
            fn += noise(x * noiseLayer.freq, y * noiseLayer.freq, noiseLayer.perm) * noiseLayer.amp;
        }
    ...
    private double noise(double x, double y, int[] layer)
    {
        int X = (int)Math.Floor(x) & 255, Y = (int)Math.Floor(y) & 255;

        double fx = fade(x - Math.Floor(x)), fy = fade(y - Math.Floor(y));

        int AA = layer[X] + Y, AB = layer[X] + Y + 1, BA = layer[X + 1] + Y, BB = layer[X + 1] + Y + 1;

        return lerp(fy, lerp(fx, perm[layer[AA]], perm[layer[BA]]), lerp(fx, perm[layer[AB]], perm[layer[BB]]));
    }
}